...
class EncoderText
{
	...
	*/
	...
	private function getCharacterCode($char, $submode)
	{
		if (!isset($this->reverseLookup[$char])) {
		$ord = ord($char);
			throw \PDF417\pException::EncoderError("Character [$char] (ASCII $ord) cannot be encoded.");
		}

		if (!isset($this->reverseLookup[$char][$submode])) {
		$ord = ord($char);
			throw \PDF417\pException::EncoderError("Character [$char] (ASCII $ord) cannot be encoded in submode [$submode].");
		}
	...
	*/
	private function getSwitchCodes($from, $to)
	{
		if (!isset($this->switching[$from][$to])) {
			throw \PDF417\pException::EncoderError("Cannot find switching codes from [$from] to [$to].");
		}
